...
def _vector_recall(query: str, doc_type: Optional[str] = None,
                   top_k: int = None, query_embedding=None,
...
class _BM25Index:
    ...
    def score(self, query_tokens: List[str], doc_type: Optional[str] = None,
...
def _rewrite_recall(original_query: str, doc_type: Optional[str] = None,
                    top_k: int = None,
                    resume_summary: str = "",
                    jd_summary: str = "",
                    visible_doc_ids: set[str] | None = None) -> List[Dict]:
    ...
    for rq in rewrite_queries:
        ...
        try:
            vec_results = _vector_recall(
                rq.query_text, doc_type=doc_type, top_k=top_k,
...
def multi_recall(
    ...
    rewrite_results = _rewrite_recall(
        query, doc_type=doc_type, top_k=top_k,
        resume_summary=resume_summary,
        jd_summary=jd_summary,
        visible_doc_ids=visible_doc_ids,
    )
...
def _bm25_score(query: str, doc_type: Optional[str] = None,
                top_k: int = None) -> List[Tuple[str, float]]:
