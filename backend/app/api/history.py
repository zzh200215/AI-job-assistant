...
def _visible_job_map(db: Session, user: User, jd_ids: list[int]) -> dict[int, JobDescription]:
    if not jd_ids:
        return {}
    return {job.id: job for job in accessible_job_query(db, user).filter(JobDescription.id.in_(jd_ids)).all()}
    ...
    return ok(
        {
            ...
            "jd_id": rec.jd_id,
            "resume": {"id": r.id, "name": r.name, "file_name": r.file_name, "parsed": r.parsed_json or {}}
            if r
            else None,
            "jd": {"id": j.id, "title": j.title, "company": j.company, "parsed": j.parsed_json or {}} if j else None,
