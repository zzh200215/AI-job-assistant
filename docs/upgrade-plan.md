...
**验证**：把一条坏串重新种回去 → 新规则红；还原 → 绿（非空性）。后端控制用例要求"旧的那行必须判出、同一份真实语料里的正常注释不得判出"，与主判据共用同一套字频。`pytest` **724 → 726**，`ruff check` + `format --check` 300 文件 clean；`test:unit` **90 → 91**，lint 0 error，smoke 11，build 通过；顺手把 D8 那条超长行折成 prettier 期望的形状，`styleDebtRatchet.test.js` 首次本地 prettier clean。**没验**：文案变长/变短后的排版（乱码串普遍比正字宽，没做真浏览器对比）；`.md`/`.json` 等其它文件类型不在守卫覆盖面内。
