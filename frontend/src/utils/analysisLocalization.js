const EN_TEXT_FALLBACKS = {
  'Proceed with further interviews to assess cultural fit and potential for training.': '建议继续安排后续面试，重点评估文化匹配度以及培养潜力。',
  ...
  'Limited exposure to the specific industry mentioned in the job description.': '对岗位描述中提到的目标行业接触还不够深入。',
  'Lack of certain specialized skills required for the role': '仍缺少岗位所要求的部分专项技能。',
  'Some required skills are not explicitly mentioned in the resume.': '部分岗位所需技能尚未在简历中被明确体现出来。',
  "Educational background could be more aligned with the job's requirements.": '教育背景与岗位要求的贴合度仍可进一步提升。',
  'Possible challenges in meeting educational criteria without further qualifications.': '若不进一步补充相关资质，可能较难完全满足学历或教育条件要求。',
  ...
  'Potential need for additional training or onboarding to bridge the industry knowledge gap.': '可能需要通过额外培训或入职辅导来补齐行业知识差距。',
...
export const localizeSentence = (value) => {
  ...
  if (lower.includes('lack of certain specialized skills') && lower.includes('required for the role')) {
    return '仍缺少岗位所要求的部分专项技能。'
  }
  if (lower.includes('required skills') && lower.includes('explicitly mentioned') && lower.includes('resume')) {
  ...
  if (lower.includes('mismatch in understanding') && lower.includes('specialized tools or processes')) {
  ...
  if (lower.includes('consider the candidate for the role') && lower.includes('industry-specific training')) {
...
export const normalizeLocalizedObjectList = (value) => {
  ...
  return value.map((item) => {
    ...
    return {
      ...item,
      item: localizeSentence(item.item || item.name || item.title || item.skill || item.point || ''),
...
export const normalizeLocalizedTextList = (value) => {
  ...
  return value.map((item) => {
    ...
    if (item && typeof item === 'object') {
      return localizeSentence(item.item || item.name || item.title || item.point || item.text || '')
    }
    return String(item || '')
  }).filter(Boolean)
}
