<template>
  <div class="page">
    ...
    <el-card>
      <template #header>
        <span><el-icon><Connection /></el-icon> 🤖 多智能体协作分析</span>
      </template>

      <el-alert title="说出你的需求，AI 自动决定调用哪些智能体" type="success" :closable="false" show-icon class="mb">
        <template #default>
      ...
      <el-collapse class="adv">
        <el-collapse-item title="⚙️ 高级：手动指定简历 / JD ID（可选）">
          <el-form :inline="true">
    ...
    <!-- 调度决策卡 -->
    <el-card v-if="runId && dispatch" class="mt">
      <template #header>
        <span><el-icon><MagicStick /></el-icon> 🧭 智能调度决策</span>
      </template>
      ...
      <el-row :gutter="12">
        <el-col :span="8" v-for="agent in agents" :key="agent.name" class="mb">
          <el-card
            ...
            :shadow="agent.status === 'running' ? 'always' : 'hover'"
          >
            <template #header>
    ...
    <!-- 汇总报告 -->
    <el-card v-if="summaryReport" class="mt">
      <template #header>
        <span><el-icon><DocumentChecked /></el-icon> {{ summaryReport.report_title || '最终汇总报告' }}</span>
      </template>
    ...
    <!-- 轮询中 -->
    <el-card v-if="runId && !isComplete" class="mt">
...
let pollCount = 0
let pollErrors = 0
const MAX_POLLS = 400        // 1.5s * 400 ≈ 10 分钟上限
const MAX_POLL_ERRORS = 5    // 连续 5 次请求失败则放弃
...
// 调度决策：优先取 Dispatcher 消息的完整输出，回退到 run 上的字段
...
// 本次实际调用的智能体（调度完成后由后端回写；未就绪时先按全量占位）
...
const agentStatusText = (agent) => {
  ...
  if (agent.status === 'running') return '⚙️ 执行中…'
...
.agent-card.running { border-color: #E6A23C; }
.agent-card.completed { border-color: #67C23A; }
.agent-card.failed { border-color: #F56C6C; }
.agent-header   { display: flex; justify-content: space-between; align-items: center; }
.agent-time     { color: #909399; font-size: 11px; }
.agent-status   { display: flex; align-items: center; gap: 6px; font-size: 13px; margin-bottom: 6px; }
.agent-summary  { font-size: 12px; color: #606266; background: #f5f7fa; padding: 4px 8px; border-radius: 4px; }
.agent-error    { color: #F56C6C; font-size: 12px; margin-top: 4px; }
.dep-graph      { margin-top: 12px; text-align: center; font-size: 12px; color: #909399; }
.dep-line       { margin-left: 6px; }
.s-green  { color: #67C23A; }
.s-warning { color: #E6A23C; }
.s-danger { color: #F56C6C; }
.s-info   { color: #909399; }
.json-preview   { background: #1e1e1e; color: #d4d4d4; padding: 16px; border-radius: 4px;
