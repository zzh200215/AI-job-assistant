      ...
      <el-card class="score-card" shadow="never">
        <div class="score-hero">
          <div class="score-ring">
            <el-progress type="circle" :percentage="result.overall_score" :stroke-width="8" :size="140"
          ...
          <div class="score-info">
            ...
            <p class="weight-info">权重配置: skill{{ result.weights_used.skill }} / project{{ result.weights_used.project }} / exp{{ result.weights_used.experience }} / edu{{ result.weights_used.education }} / keyword{{ result.weights_used.keyword }} / bonus{{ result.weights_used.bonus }}</p>
          </div>
        </div>
      </el-card>
      ...
      <el-card shadow="never">
        ...
        <div v-for="dim in result.dimensions" :key="dim.name" class="dim-block">
          <div class="dim-header">
            ...
            <span class="dim-score" :style="{ color: scoreColor(dim.score) }">{{ dim.score.toFixed(1) }}</span>
          </div>
          <div class="dim-bar">
            <div class="dim-fill" :style="{ width: dim.score + '%', background: scoreColor(dim.score) }" />
          </div>
          <p class="dim-reason">{{ dim.reason }}</p>
          <div v-if="dim.details?.length" class="dim-details">
            <el-tag v-for="d in dim.details" :key="d" size="small" type="info" effect="plain" style="margin:1px;">{{ d }}</el-tag>
          </div>
        </div>
      </el-card>
      ...
      <el-card v-if="result.skill_match" shadow="never">
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            ...
            <el-tag v-for="s in (result.skill_match.matched || [])" :key="s" type="success" style="margin:2px;">{{ s }}</el-tag>
            <el-empty v-if="!result.skill_match.matched?.length" description="无" :image-size="40" />
          </el-col>
          <el-col :span="12">
            <h5>❌ 缺失核心 ({{ result.skill_match.missing_required?.length || 0 }})</h5>
            <el-tag v-for="s in (result.skill_match.missing_required || [])" :key="s" type="danger" style="margin:2px;">{{ s }}</el-tag>
            <el-empty v-if="!result.skill_match.missing_required?.length" description="无" :image-size="40" />
          </el-col>
        </el-row>
      </el-card>
...
<style scoped>
.em-page { max-width: 800px; margin: 0 auto; padding: 16px 0; display: flex; flex-direction: column; gap: 16px; }
.em-card { border-radius: 12px; }
...
.big-score { font-size: 32px; font-weight: 700; color: #303133; }
.score-lbl { font-size: 12px; color: #909399; }
.score-info { flex: 1; }
.overall-reason { font-size: 14px; color: #606266; margin: 8px 0; }
.weight-info { font-size: 11px; color: #909399; }
.dim-block { margin-bottom: 16px; }
.dim-header { display: flex; align-items: center; gap: 12px; }
.dim-name { font-weight: 600; width: 80px; }
.dim-w { font-size: 11px; color: #909399; }
.dim-score { font-size: 18px; font-weight: 700; }
.dim-bar { height: 10px; background: #f0f2f5; border-radius: 5px; margin: 4px 0; overflow: hidden; }
.dim-fill { height: 100%; border-radius: 5px; transition: width 0.6s; }
.dim-reason { font-size: 12px; color: #606266; }
.dim-details { margin-top: 2px; }
h5 { margin: 8px 0 4px; }
ul { padding-left: 18px; }
li { margin: 4px 0; font-size: 13px; color: #606266; }
</style>
